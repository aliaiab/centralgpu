pub fn main() !void {
    ...
    _ = sdl.SDL_SetAppMetadata("softraster", "0.0.0", "softraster");
    ...
    const target_buffer = try gpa.alloc(softraster.Rgba32, window_width * window_height);
    defer gpa.free(target_buffer);

    const target_buffer_linear = try gpa.alloc(softraster.Rgba32, window_width * window_height);
    ...
    while (true) {
        ...
        const vertex_colors: [2][3]u32 = .{
            .{
                @bitCast(softraster.Rgba32.fromNormalized(.{ 1, 0, 0, 1 })),
                @bitCast(softraster.Rgba32.fromNormalized(.{ 0, 1, 0, 1 })),
                @bitCast(softraster.Rgba32.fromNormalized(.{ 0, 0, 1, 1 })),
            },
            .{
                @bitCast(softraster.Rgba32.fromNormalized(.{ 0.4, 0, 1, 1 })),
                @bitCast(softraster.Rgba32.fromNormalized(.{ 0, 1, 0, 1 })),
                @bitCast(softraster.Rgba32.fromNormalized(.{ 1, 0.4, 0, 1 })),
            },
        };
        ...
        if (false) {
            ...
            var out_triangle: softraster.WarpProjectedTriangle = undefined;
...
const softraster = @import("root.zig");
